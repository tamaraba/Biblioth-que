	...
	public function configure_api_key( $submitted_data ) {
		...
		if ( $is_submit ) {
			...
			if ( ! $has_errors ) {
				...
				return array(
					'html'         => Hustle_Provider_Utils::get_integration_modal_title_markup( __( 'GetResponse Added', 'wordpress-popup' ), __( 'You can now go to your pop-ups, slide-ins and embeds and assign them to this integration', 'wordpress-popup' ) ),
					'buttons'      => array(
						'close' => array(
	...
	private function validate_api_key( $api_key ) {
		...
		try {
			...
			if ( is_wp_error( $campaigns ) ) {
				Hustle_Provider_Utils::maybe_log( __METHOD__, __( 'Invalid GetResponse API key.', 'wordpress-popup' ) );
				return false;
			}
