...
class Hustle_General_Data_Protection {
	...
	public function register_eraser( $erasers = array() ) {
		$erasers[ 'hustle-module-submissions' ] = array(
			'eraser_friendly_name' => __( 'Hustle Module Submissions', 'wordpress-popup' ),
	...
	public function register_exporter( $exporter = array() ) {
		$exporter[ 'hustle-module-submissions' ] = array(
			'exporter_friendly_name' => __( 'Hustle Module Submissions', 'wordpress-popup' ),
	...
	public static function do_submissions_eraser( $email, $page ) {
		...
		if( true === $erasure_disabled ) {
			...
			$response['messages'][] = __( 'Hustle submissions were retained.', 'wordpress-popup' );
			return $response;
		}
		...
		if( ! empty ( $entry_ids ) ){
			foreach ( $entry_ids as $entry_id ) {
				...
				$response['messages'][] 	= sprintf( __( 'Hustle submission #%d was deleted.', 'wordpress-popup' ), $entry_id );

			}
			$response['items_removed'] 	= true;
			$response['items_retained'] = false;
		}else{
			$response['messages'][] = __( ' Hustle submissions not found.', 'wordpress-popup' );
		}
		...
		if ( ! empty( $entry_ids ) && is_array( $entry_ids ) ) {
			foreach ( $entry_ids as $entry_id ) {
				...
				$data_to_export[] = array(
					...
					'group_label' => __( 'Hustel Module Submissions', 'wordpress-popup' ),
	...
	public static function get_custom_form_export_mappers( $model ) {
		...
		$mappers = array(
			array(
				'name'    	=> __( 'Entry ID', 'wordpress-popup' ),
				'value'	   	=> $model->entry_id
			),
			array(
				'name'    	=> __( 'Submission Date', 'wordpress-popup' ),
